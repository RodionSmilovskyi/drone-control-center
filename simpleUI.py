                ...
                if (time.time() - last_slow_msg_time) >= SLOW_MSGS_LOOP_TIME:
                    ...
                    if next_msg == "MSP_ANALOG":
                        ...
                        screen.addstr(
                            8,
                            0,
                            "Battery Voltage: {:2.2f}V".format(board.ANALOG["voltage"]),
                        )
                        screen.clrtoeol()
                        screen.addstr(
                            8, 24, voltage_msg, curses.A_BOLD + curses.A_BLINK
                        )
                        screen.clrtoeol()
                        ...
                        screen.addstr(
                            6, 0, "cpuload: {}".format(board.CONFIG["cpuload"])
                        )
                        screen.clrtoeol()
                        screen.addstr(
                            6, 50, "cycleTime: {}".format(board.CONFIG["cycleTime"])
                        )
                        screen.clrtoeol()
                        ...
                        screen.addstr(
                            10,
                            0,
                            "Attitude: {}".format(board.SENSOR_DATA["kinematics"]),
                        )
                        screen.clrtoeol()

                    elif next_msg == "MSP_ALTITUDE":
                        screen.addstr(
                            11, 0, "Altitude: {}".format(board.SENSOR_DATA["altitude"])
                        )
                        screen.clrtoeol()

                    elif next_msg == "MSP_RC":
                        screen.addstr(
                            12, 0, "RC Channels Values: {}".format(board.RC["channels"])
                        )
                        screen.clrtoeol()
                    ...
                    screen.addstr(
                        13,
                        0,
                        "GUI cycleTime: {0:2.2f}ms (average {1:2.2f}Hz)".format(
                            (last_cycleTime) * 1000,
