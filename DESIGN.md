...
- **Operating Modes:**
  ...
  - `ai`: Computes PID-controlled throttle to maintain dynamic altitude setpoint (default `0.4m`, bounded `[0.1, 2.5m]`, paired with Betaflight `vbat_sag_compensation = 100` and hover throttle baseline = `1540`, bounded `[1341, 1800]`) alongside roll/pitch/yaw commands. Altitude is normalized via $z_{\text{norm}} = z / \text{MAX\_ALTITUDE}$ (`MAX_ALTITUDE = 3.0m`), mapped to high-level action range $[-1, 1]$ via $a_{\text{alt}} = 2 \cdot z_{\text{norm}} - 1$. The PID controller incorporates derivative-on-measurement with persistent velocity tracking across duplicate sensor frames, clamped frame $\Delta t \in [1, 50]\text{ms}$ with smooth setpoint slew/ramping ($0.25\text{ m/s}$) protected by a ground anchor (clamping setpoint to $\le z_{\text{current}} + 0.02\text{m}$ while resting on the deck to prevent pre-launch error runaway), ground unweight threshold anti-windup (freezing integral below 0.020m), asymmetrical PID correction authority (effective throttle range $[1465, 1680]\text{ PWM}$ via descent floor $-75\text{ PWM}$ and climb ceiling $+140\text{ PWM}$) providing active braking against fresh-battery overshoots while preserving sag headroom, and an adaptive non-negative integral term ($K_i = 1.0$, clamped to $[0.0, 1.2]$ / up to $+120\text{ PWM}$ trim) paired with $K_p = 7.5$ and $K_d = 2.8$ for rock-solid stability. Sensor altitude filtering utilizes $\alpha = 0.6$ for low-latency feedback.
- **Output:** Publishes RC packet `[roll, pitch, throttle, yaw, aux1, aux2]` over ZMQ PUB `tcp://127.0.0.1:5556`.

#### C. `drone-fc` (`services/drone-fc/`)
- **Execution Rate:** 100 Hz.
- **Hardware Interface:** Communicates with Betaflight/iNAV via serial using MultiWii Serial Protocol (MSP) via `yamspy.MSPy`. Features dynamic serial port detection (`/dev/ttyACM*`, `/dev/ttyUSB*`), multi-attempt initialization with DTR settle times to survive service restarts, and safe disarm sequence upon shutdown.
