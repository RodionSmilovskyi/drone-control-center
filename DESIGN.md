...
- **Operating Modes:**
  ...
  - `ai`: Computes PID-controlled throttle to maintain dynamic altitude setpoint (default `0.4m`, bounded `[0.1, 2.5m]`, paired with Betaflight `vbat_sag_compensation = 100` and hover throttle baseline = `1555`, bounded `[1341, 1800]`) alongside roll/pitch/yaw commands. Altitude is normalized via $z_{\text{norm}} = z / \text{MAX\_ALTITUDE}$ (`MAX_ALTITUDE = 3.0m`), mapped to high-level action range $[-1, 1]$ via $a_{\text{alt}} = 2 \cdot z_{\text{norm}} - 1$. The PID controller incorporates derivative-on-measurement with persistent velocity tracking across duplicate sensor frames, clamped frame $\Delta t \in [1, 50]\text{ms}$ with smooth setpoint slew/ramping ($0.25\text{ m/s}$) for catapult-free soft takeoff, ground unweight threshold anti-windup (freezing integral below 0.020m), asymmetrical PID correction authority (tight descent floor $-50$ PWM, expanded climb headroom $+130$ PWM, effective throttle range $[1505, 1685]$) to eliminate ground-effect bounces while providing battery sag compensation across full discharge, and an adaptive integral term ($K_i = 0.4$, limit $\pm 48$ PWM / `integral_limit = 1.2`) with increased damping ($K_d = 2.8$) for rock-solid stability and automatic hover throttle trimming. Sensor altitude filtering utilizes $\alpha = 0.6$ for low-latency feedback.
